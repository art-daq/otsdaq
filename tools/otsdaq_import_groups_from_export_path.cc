...
void ImportTableGroupsFromPath(int argc, char* argv[])
{
	...
	if(!(fp = fopen(ConfigurationManager::ACTIVE_GROUPS_FILENAME.c_str(), "r")))
	{
		...
		if(!foundExistingEmptyBackbone)  //save new empty backbone
		{
			...
			while(1)
			{
				__COUT__ << "New Key for empty backbone group: " << importedBackboneGroupName
				...
				try
				{
					theInterface_->saveTableGroup(
						backboneMemberMap,
						TableGroupKey::getFullGroupString(importedBackboneGroupName, newKey));
				}
				catch(const std::runtime_error& e)
				{
					...
					if(std::string(e.what()).find("there was a collision") != std::string::npos)
					{
						__COUT_WARN__ << "There was a collision saving the new group " <<
							importedBackboneGroupName << "(" << newKey << "), trying incremented group key... retries=" << retries << __E__;
						if(++retries > 10) //give up
							throw;
						newKey = TableGroupKey::getNextKey(newKey); //increment group key
						__COUT__ << "New Key for group: " << importedBackboneGroupName << " found as " << newKey << __E__;
						continue;
					}
					else throw;
				}
	...
	for(auto& group : importGroupMap)
	{
		...
		for(auto& member : memberTableSet)
		{
			...
			auto memberTable =
			    member.first == TableBase::GROUP_METADATA_TABLE_NAME
					...
					catch(const std::runtime_error& e)
					{
						...
						if(std::string(e.what()).find("there was a collision") != std::string::npos)
						{
							__COUT_WARN__ << "There was a collision saving the new table " <<
								*tableView << "(" << newAssignedVersion << "), trying incremented table version... retries=" << retries << __E__;
							if(++retries > 10) //give up
								throw;
							newAssignedVersion = TableVersion::getNextVersion(newAssignedVersion); //increment table version
							tableView->setVersion(newAssignedVersion);
							__COUT__ << "New version for table: " << *tableView << " found as " << newAssignedVersion << __E__;
							continue;
						}
						else throw;
					}
					
					__COUT__ << "Created table: " << tableView << "-v" << newAssignedVersion << __E__;
					break;
				} //end collission retry loop
		...
		{	
			...
			catch(const std::runtime_error& e)
			{
				...
				{
					__COUT_WARN__ << "There was a collision saving the new group " <<
						group.first.first << "(" << newKey << "), trying incremented group key... retries=" << retries << __E__;
					if(++retries > 10) //give up
						throw;
					newKey = TableGroupKey::getNextKey(newKey); //increment group key
