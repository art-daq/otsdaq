...
// clang-format on
	...
	// add groups to vector list from directory
	{
		...
		{
			__COUT_ERR__ << "ERROR:(" << errno << ").  Can't open directory: " << importPath
			         << std::endl;
			exit(0);
		}
		...
		while((dirp = readdir(dp)) != 0)
			if(dirp->d_type == isDir && dirp->d_name[0] != '.') //if a directory type
			{
				...
				importGroupMap.insert(std::pair<std::pair<std::string /*groupName*/, TableGroupKey /*origKey*/>,
					 TableGroupKey /*newKey*/>(
					std::pair<std::string, TableGroupKey>(split[0],
														split[1]),
					TableGroupKey()));

				//check if group is the active backbone to import
				std::string groupIsBackbonePath = importPath + "/" + dirp->d_name + "/" + "groupIsBackbone.txt";
	...
	if(!(fp = fopen(ConfigurationManager::ACTIVE_GROUPS_FILENAME.c_str(),"r")))
	{
		...
		__COUT_INFO__ << "Creating an empty Backbone group... group name will match imported backbone name: " << importedBackboneGroupName << __E__;
		...
		{ //check if group is a duplicate 
			...
			try
			{
				TableGroupKey foundKey =
					cfgMgr->findTableGroup(importedBackboneGroupName, backboneMemberMap); //, memberTableAliases); std::map<std::string /*name*/, std::string /*alias*/>    memberTableAliases
				
				if(!foundKey.isInvalid())
				{
					__COUT_WARN__ << "Found equivalent empty backbone group key (" << foundKey << ") for "
							<< importedBackboneGroupName << ". Skipping creation and activating existing key!" << __E__;		
				...
				__COUT__ << "Check for empty backbone duplicate groups complete." << __E__;
			}
			catch(...)
			{
				__COUT_WARN__ << "Ignoring errors looking for empty backbone duplicate groups! Proceeding "
		...
		{
			TableGroupKey newKey =
	    		TableGroupKey::getNextKey(theInterface_->findLatestGroupKey(importedBackboneGroupName));
			__COUT__ << "New Key for empty backbone group: " << importedBackboneGroupName << " found as " << newKey << __E__;

			theInterface_->saveTableGroup(
		   		backboneMemberMap, TableGroupKey::getFullGroupString(importedBackboneGroupName, newKey));
			__COUT__ << "Created new empty backbone table group: " << importedBackboneGroupName << ":" << newKey << __E__;
	...
	{
		__SS__ << "Did not find valid active groups in current database as starting point for import! "
		          "Must have a backbone at least. Is the current database URI correct? ARTDAQ_DATABASE_URI = " 
				  << ARTDAQ_DATABASE_URI
				  << "\n\n*** Note: The active groups are specified by the Active Groups file: " << ConfigurationManager::ACTIVE_GROUPS_FILENAME 
				  << "\n*** If you want an empty backbone to be generated for you as a starting point, please delete the Active Groups file."
		       << std::endl;
		__SS_THROW__;
	}
	...
	{ //check import backbone group and table aliases for uniqueness
		std::string importBackbonePath = importPath + "/" + importedBackboneGroupName + "_" + importedBackboneGroupKey.str();
		__COUTV__(importBackbonePath);
				
		std::map<std::string /* tableName */, TableVersion /* tableVersion */> memberTableSet;
		{ //get member table file names
			DIR* dp;
			struct dirent* dirp;
			if((dp = opendir(importBackbonePath.c_str())) == 0)
			{
				__COUT_ERR__ << "ERROR:(" << errno << ").  Can't open directory: " << importBackbonePath
						<< std::endl;
				exit(0);
			}
			...
			while((dirp = readdir(dp)) != 0)
				if(dirp->d_type != isDir && strlen(dirp->d_name) > 5 && 
					...
					dirp->d_name[strlen(dirp->d_name)-1] == 'n') //if not directory w/extension .json
				{
					...
					memberTableSet.insert(std::pair<std::string, TableVersion>(
							split[0],
							split[1].substr(1,split[1].size()-6)));					
				} //end found group directory handling
		...
		{ //verify group aliases are unique
			...
			__COUTV__(cfgMgr->getNode(ConfigurationManager::GROUP_ALIASES_TABLE_NAME).getTableName());
			__COUTV__(cfgMgr->getNode(ConfigurationManager::GROUP_ALIASES_TABLE_NAME).getTableVersion());
			...
			for(auto& groupPair : currentAliasNodePairs)
			{
				__COUT__ << " ==> Current group alias " << 
						...
						TableGroupKey(groupPair.second.getNode("GroupKey").getValueAsString()) << ")" << __E__;

				currentAliases.emplace(groupPair.second.getNode("GroupKeyAlias").getValueAsString());
			}
			
			std::string fullpath = importBackbonePath + "/" + ConfigurationManager::GROUP_ALIASES_TABLE_NAME + "_v" + 
				memberTableSet.at(ConfigurationManager::GROUP_ALIASES_TABLE_NAME).str() + ".json";
			...
			auto aliasTable = cfgMgr->getTableByName(ConfigurationManager::GROUP_ALIASES_TABLE_NAME);
			...
			for(size_t row = 0; row < aliasView->getNumberOfRows(); ++row)
			{
				...
				__COUT__ << " ==> Group alias to import at row " << row << ": " << aliasName << __E__;
				if(currentAliases.find(prepend + aliasName) != 
					currentAliases.end())
				{
					__SS__ << "The imported prepend + group alias of '" << prepend << "' + '" << 
						aliasName << 
						"' already exists as a group alias in the current database." <<
						" Please modify the prepend string to make the imported alias names unique." << __E__;
					__SS_THROW__;
				}
				importGroupAliasMap[aliasName] = std::pair<std::string /*groupName*/, TableGroupKey /*origKey*/>(
		...
		{ //verify table aliases are unique
			...
			__COUTV__(cfgMgr->getNode(ConfigurationManager::VERSION_ALIASES_TABLE_NAME).getTableName());
			__COUTV__(cfgMgr->getNode(ConfigurationManager::VERSION_ALIASES_TABLE_NAME).getTableVersion());

			__COUT__ << "Existing table aliases: " << currentAliasNodePairs.size() << __E__;
			for(auto& groupPair : currentAliasNodePairs)
			{
			...
			std::string fullpath = importBackbonePath + "/" + ConfigurationManager::VERSION_ALIASES_TABLE_NAME + "_v" + 
				memberTableSet.at(ConfigurationManager::VERSION_ALIASES_TABLE_NAME).str() + ".json";
			...
			auto aliasTable = cfgMgr->getTableByName(ConfigurationManager::VERSION_ALIASES_TABLE_NAME);
			auto temporaryVersion = aliasTable->createTemporaryView();
			auto aliasView = aliasTable->getViewP(temporaryVersion);
			...
			{
				std::string aliasName = aliasView->getValueAsString(row,aliasCol);
				__COUT__ << " ==> Table alias to import at row " << row << ": " << aliasName << __E__;
				if(currentAliases.find(prepend + aliasName) != 
					currentAliases.end())
				{
				...
				importTableAliasMap[aliasName] = std::pair<std::string /*tableName*/, TableVersion /*origVersion*/>(
	...
	for(auto& group : importGroupMap)
	{
		...
		std::string importGroupPath = importPath + "/" + group.first.first + "_" + group.first.second.str();
		...
		{ //get member table file names
			...
			{
				__COUT_ERR__ << "ERROR:(" << errno << ").  Can't open directory: " << importGroupPath
						<< std::endl;
				exit(0);
			}
		...
		for(auto& member : memberTableSet)
		{
			...
			auto memberTable = member.first == ConfigurationInterface::GROUP_METADATA_TABLE_NAME?
			 	cfgMgr->getMetadataTable() : cfgMgr->getTableByName(member.first);
			

			TableVersion newAssignedVersion;
			if(member.first == ConfigurationInterface::GROUP_METADATA_TABLE_NAME)
			{
			...
			importTableMap[std::make_pair(member.first,member.second)] = newAssignedVersion; 
		...
		__COUT__ << "Saving group '" << group.first.first << "' members: " << groupMembers.size() << std::endl;
		...
		for(auto& table : groupMembers)
		{
			...
			if(table.first != ConfigurationInterface::GROUP_METADATA_TABLE_NAME)
		...
		{ //check if group is a duplicate 
			...
			{
				TableGroupKey foundKey =
					cfgMgr->findTableGroup(group.first.first, groupMembersWithoutMeta); //, memberTableAliases); std::map<std::string /*name*/, std::string /*alias*/>    memberTableAliases
				
				if(!foundKey.isInvalid())
				{
					__COUT_WARN__ << "Found equivalent group key (" << foundKey << ") for "
							<< group.first.first << ". Skipping import!" << __E__;	
					//update key import transformation map
					importGroupMap.at(std::make_pair(group.first.first, group.first.second)) = foundKey;					
					continue;					
				}
		...
		TableGroupKey newKey =
	    	TableGroupKey::getNextKey(theInterface_->findLatestGroupKey(group.first.first));
		__COUT__ << "New Key for group: " << group.first.first << " found as " << newKey << __E__;
		
		theInterface_->saveTableGroup(
		    groupMembers, TableGroupKey::getFullGroupString(group.first.first, newKey));
		anyNewGroupSaved = true;
		
		__COUT__ << "Created table group: " << group.first.first << ":" << newKey << __E__;
	...
	if(!anyNewGroupSaved)
	{
		__SS__ << "All groups to import already exist in current db! Was the wrong db selected from which to import?" << __E__;	
		__SS_THROW__; 
	}
	...
	//		- should be basename+alias connection to (hop through maps) new groupName & groupKey
	{ //insert new aliases for imported groups in current active backbone	
		auto table = cfgMgr->getTableByName(ConfigurationManager::GROUP_ALIASES_TABLE_NAME);
		__COUTV__(table->getTableName());
		__COUTV__(table->getViewVersion());
	...
	std::map<std::string, TableVersion>  backboneMemberMap;
	{
		...
		cfgMgr->restoreActiveTableGroups(false /*throwErrors*/,
		                                 "" /*pathToActiveGroupsFile*/,
		                                 ConfigurationManager::LoadGroupType::ALL_TYPES, //must do ALL_TYPES to not affect activeTablesGroup file
		                                 &accumulatedWarnings);

		activeBackboneGroupName = cfgMgr->getActiveGroupName(ConfigurationManager::GroupType::BACKBONE_TYPE);
		...
		__COUT__ << "Done re-loading active backbone: " << activeBackboneGroupName << 
			" (" << cfgMgr->getActiveGroupKey(ConfigurationManager::GroupType::BACKBONE_TYPE) << ")" << std::endl;
	}
	...
	{ //insert new aliases for imported groups in current active backbone	
		...
		{
			__COUT__ << "\t" << aliasPair.first << " ==> " << aliasPair.second.first << "-v"
		         << aliasPair.second.second << std::endl;
