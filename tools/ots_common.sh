...
do_out() {
	...
	if [ "$tlvl" == "FIRST" ]; then #do special header printout
		if hash trace_cntl >/dev/null 2>&1;then
			TRACE_PRINT="${part1}      TIME      $part2 %n:${Cyan}LINE${RstClr} $part3" \
			trace_cntl -n`basename "${BASH_SOURCE[2]}"` -L${BASH_LINENO[1]} TRACE TLVL_INFO "$(echo -e "$*")${RstClr}"
		else
			echo -e "${part1} TIME $part2 `basename "${BASH_SOURCE[2]}"`:${Cyan}LINE${RstClr} ${part3}${*}${RstClr}"
		fi
	else #normal printouts
		if hash trace_cntl >/dev/null 2>&1;then
