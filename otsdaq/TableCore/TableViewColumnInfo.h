...
class TableViewColumnInfo
{
  public:
	...
	static const std::string&       getDefaultDefaultValue(const std::string& type,
	                                                       const std::string& dataType);
	...
	struct BitMapInfo  ///< uses dataChoices CSV fields if type is TYPE_BITMAP_DATA
	{
		...
		bool         rowsAscending_, colsAscending_, snakeRows_, snakeCols_,
		    floatingPoint_ = false, mapsToStrings_ = false;
	};
	...
	static const std::string COL_NAME_STATUS, COL_NAME_ENABLED, COL_NAME_PRIORITY,
	    COL_NAME_COMMENT, COL_NAME_AUTHOR, COL_NAME_CREATION;

  private:
	TableViewColumnInfo();  ///< private constructor, only used in assignment operator
	void extractBitMapInfo();

	std::vector<std::string> getDataChoicesFromString(
