...
class TableView
{
  public:
	...
	unsigned int copyRows(const std::string& author,
	                      ...
	                      unsigned char      generateUniqueDataColumns =
	                          false,  ///< leave as unsigned char rather than
	...
	unsigned int findColByType(const std::string& type,
	                           unsigned int       startingCol = 0) const;
	...
	{
		return comment_ == "" ? TableViewColumnInfo::DATATYPE_COMMENT_DEFAULT : comment_;
	}
  ...
  public:
	std::set<std::string /*groupID*/> getSetOfGroupIDs(const std::string& childLinkIndex,
	                                                   unsigned int       row = -1) const;
	std::set<std::string /*groupID*/> getSetOfGroupIDs(const unsigned int& col,
	                                                   unsigned int row = -1) const;
	...
	std::vector<unsigned int /*row*/> getGroupRows(const unsigned int groupIdCol,
	                                               const std::string& groupID,
	                                               bool onlyStatusTrue    = false,
	                                               bool orderedByPriority = false) const;
	std::vector<std::vector<unsigned int /*row*/>> getGroupRowsByPriority(
	    const unsigned int groupIdCol,
	    const std::string& groupID,
	    bool               onlyStatusTrue = false) const;

  private:
	std::vector<std::vector<unsigned int /*row*/>> getGroupRowsInVectors(
	    const unsigned int groupIdCol,
	    const std::string& groupID,
	    bool               onlyStatusTrue,
  ...
  public:
	...
	const std::string& setUniqueColumnValue(
	    unsigned int row,
	    unsigned int col,
	    std::string  baseValueAsString    = "",
	    bool         doMathAppendStrategy = false,
	    std::string  childLinkIndex =
	        "",  ///<to allow for handling TableViewColumnInfo::TYPE_UNIQUE_GROUP_DATA
	    std::string groupId = "");
	...
	void           printCSV(std::ostream&      out                = std::cout,
  ...
  private:
	...
	std::string
	    storageData_;  ///< starts empty "", used to implement re-writable views ("temporary views") in artdaq db
	...
	time_t
	    lastAccessTime_;  ///< last time the ConfigurationInterface:get() retrieved this view
