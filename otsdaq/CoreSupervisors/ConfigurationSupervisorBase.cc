...
try
{
	...
	if(0)
	{
		...
		{
			std::map<std::string, TableVersion> activeTables =
		...
		try
		{
			...
			iconUID = iconTable.tableView_
			              ->getDataView()[row][iconTable.tableView_->getColUID()];
			...
			iconTable.tableView_->setValueAsString(
			    enforceOneWindowInstance ? "1" : "0",
			    row,
			    iconTable.tableView_->findCol(
			        DesktopIconTable::COL_FORCE_ONLY_ONE_INSTANCE));
			// set permissions value
			iconTable.tableView_->setURIEncodedValue(
			    iconPermissions,
			    row,
			...
			if(windowLinkedAppLID > 0)
			{
				...
				int appRow = appTable.tableView_->findRow(
				    appTable.tableView_->findCol(
				        XDAQContextTable::colApplication_.colId_),
				    windowLinkedAppLID);
				windowLinkedApp =
				    appTable.tableView_
				        ->getDataView()[appRow][appTable.tableView_->getColUID()];
			...
			if(parameters.size())
			{
				...
				iconTable.tableView_->setValueAsString(
				    ...
				    row,
				    iconTable.tableView_->findCol(
				        DesktopIconTable::COL_PARAMETER_LINK_GID));
		...
		catch(...)
		{
			...
			{
				__COUT__ << "Erasing temporary version " << parameterTable.tableName_
		...
		{
			newContextKey =
			    cfgMgr->saveNewTableGroup(contextGroupName, contextGroupMembers);
		...
		{  // check version aliases
			...
			for(auto& aliasNodePair : aliasNodePairs)
			{
				...
				{
					__COUT__ << "Found alias! Changing icon table version alias."
					         << __E__;
