...
void OtsDataSaverConsumer::writePacketHeader(const std::string& data)
{
	...
	if(quadWordsCount)
	{
		...
		__CFG_COUT__ << "quadcount: " << quadWordsCount << " packetType: " << data[0]
		             << " sequence id: " << seqId << " last: " << lastSeqId_ << __E__;
		if(!(lastSeqId_ + 1 == seqId || (lastSeqId_ == 255 && seqId == 0)))
		{
			__CFG_COUT__ << "?????? NOOOO Missing Packets: " << (unsigned int)lastSeqId_
