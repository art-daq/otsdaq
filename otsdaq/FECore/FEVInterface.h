...
#define PLOTLY_PLOT \
	"Plotly_Plot"  ///< arg out for built-in auto plotly plotting at client, for example:
...
class FEVInterface : public WorkLoop, public Configurable, public VStateMachine
{
  ...
  public:
	                                       ...
	                                       unsigned int numberOfBytes)
	{
		throw std::runtime_error(
		    "UNDEFINED BLOCK READ"); /* to make compiler happy, use params */
		__COUTV__((void*)address);
		__COUTV__((void*)returnValue);
		__COUTV__(numberOfBytes);
	}
	bool universalBlockReadImplementationConfirmed =
	    false;  ///<is confirmed by slow controls handling (for example) that universalBlockRead is implemented by the FE plugin
	virtual void universalBlockWrite(char*        address,
	                                 char*        writeValue,
	                                 unsigned int numberOfBytes)
	{
		throw std::runtime_error(
		    "UNDEFINED BLOCK WRITE"); /* to make compiler happy, use params */
		__COUTV__((void*)address);
		__COUTV__((void*)writeValue);
		__COUTV__(numberOfBytes);
	}

	void runSequenceOfCommands(const std::string& treeLinkName);

	static void sendAsyncExceptionToGateway(FEVInterface*      fe,
	...
	void         addSlowControlsChannels(
	            ConfigurationTree slowControlsGroupLink,
	            std::map<std::string /* ROC UID*/, FESlowControlsChannel>*
	                mapOfSlowControlsChannels);
	...
	struct frontEndMacroStruct_t  ///< members fully define a front-end macro function
	{
		frontEndMacroStruct_t(
		    ...
		    const std::string&              requiredUserPermissions =
		        "1" /*Level definition: 0:no-access,1:=user,255:=admin*/,
  ...
  protected:
	...
	std::string
	    mfSubject_;  ///< for __GEN_COUT__ decorations which would be safe in destructors, e.g. mirror interfaceUID_
  ...
  public:
	template<class T>
	static std::string& emplaceFEMacroArgumentValue(frontEndMacroArgs_t args,
	                                                const std::string&  argName,
	                                                const T&            value);
