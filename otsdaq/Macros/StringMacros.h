...
struct StringMacros
{
  ...
  public:
	...
	static std::string        escapeString(std::string inString,
	...
	static const std::string
	    TBD;  //for to-be-defined system variables (so there is a value in wiz mode, before configuration, etc.)

	static bool isNumber(
	    const std::string&
	        stringToCheck);  ///< Note: before call consider use of stringToCheck = StringMacros::convertEnvironmentVariables(stringToCheck)
	static std::string getNumberType(
	    const std::string&
	        stringToCheck);  ///< Note: before call consider use of stringToCheck = StringMacros::convertEnvironmentVariables(stringToCheck)
	template<class T>
	static bool getNumber(
	    const std::string& s,
	    T& retValue);  ///< defined in included .icc source // Note: before call consider use of stringToCheck = StringMacros::convertEnvironmentVariables(stringToCheck)
	///template<>
	static bool getNumber(
	    const std::string& s,
	    bool&
	        retValue);  ///< defined in included .icc source // Note: before call consider use of stringToCheck = StringMacros::convertEnvironmentVariables(stringToCheck)
	...
	static bool extractCommonChunks(const std::vector<std::string>& haystack,
	                                std::vector<std::string>&       commonChunksToReturn,
	                                std::vector<std::string>&       wildcardStrings,
	...
	{
		return StringMacros::demangleTypeName(typeid(T).name());
	}
	...
	{  ///<get string in order ignoring letter case
		bool operator()(const std::string& lhs,
		                const std::string& rhs) const;  ///<comparison handler
