		...
		if(!appLink.isDisconnected())
		{
			...
			if(icon->windowContentURL_.size() && icon->windowContentURL_[0] == '/')
			{
				...
				try
				{
					...
					if(!appLink.isEnabled())  //demoting off apps, from exception to error, to make it less annoying for users that are disabling apps
						__COUT_WARN__ << "Warning! The target app '" << appLink.getValueAsString() << 
							  "' is disabled, which will likely break the behavior of the Desktop Icon '" << child.first << ".' To fix, reenable the target app." << __E__;
					...
					if(contextUID != gatewayContextUID)
					{
						...
						if(!contextNode.isEnabled())  //demoting off apps, from exception to error, to make it less annoying for users that are disabling apps
							__COUT_WARN__ << "Warning! The parent context '" << contextUID << "' of the target app '" << appLink.getValueAsString() << 
								"' is disabled, which will likely break the behavior of the Desktop Icon '" << child.first << ".' To fix, reenable the target app's parent context." << __E__;				
