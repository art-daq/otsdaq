    ...
    : virtual public TableBase  ///<virtual so future plugins can inherit from multiple table base classes
{
  public:
	...
	virtual void genFlatFHiCL(void)
	{
		__SS__ << "genFlatFHiCL() Not defined!";
		__SS_THROW__;
	};
	...
	static const std::string ARTDAQ_READER_TABLE, ARTDAQ_BUILDER_TABLE,
	    ARTDAQ_LOGGER_TABLE, ARTDAQ_DISPATCHER_TABLE, ARTDAQ_MONITOR_TABLE,
	    ARTDAQ_ROUTER_TABLE;
	static const std::string ARTDAQ_SUBSYSTEM_TABLE, ARTDAQ_DAQ_TABLE,
	    ARTDAQ_DAQ_PARAMETER_TABLE, ARTDAQ_ART_TABLE;
	static const std::string ARTDAQ_TYPE_TABLE_HOSTNAME,
	    ARTDAQ_TYPE_TABLE_ALLOWED_PROCESSORS, ARTDAQ_TYPE_TABLE_SUBSYSTEM_LINK,
	...
	static std::string        getBootFileContentFromInfo(const ARTDAQInfo&  info,
	...
	static void        flattenFHICL(ARTDAQAppType      type,
	...
	static std::string insertModuleType(std::ostream&      out,
	                                    std::string&       tabStr,
	                                    std::string&       commentStr,
	                                    const std::string& parentPath,
	                                    ConfigurationTree  moduleTypeNode);
	static void        insertMetricsBlock(std::ostream&      out,
	                                      std::string&       tabStr,
	                                      std::string&       commentStr,
	                                      const std::string& parentPath,
	                                      ConfigurationTree  daqNode);
	static void        insertArtProcessBlock(
	           std::ostream&      out,
	           std::string&       tabStr,
	           std::string&       commentStr,
	           const std::string& parentPath,
	...
	static void outputBoardReaderFHICL(
	...
	static void outputRoutingManagerFHICL(
	...
	static const ARTDAQInfo& extractARTDAQInfo(
	    ConfigurationTree artdaqSupervisorNode,
	...
	static const ARTDAQInfo& getARTDAQSystem(
  ...
  private:
	...
	static void extractRoutingManagersInfo(ConfigurationTree artdaqSupervisorNode,
	                                       bool              getStatusFalseNodes,
	                                       bool              doWriteFHiCL,
	                                       size_t            routingTimeoutMs,
	                                       size_t            routingRetryCount);
	static void extractBoardReadersInfo(ConfigurationTree artdaqSupervisorNode,
	                                    bool              getStatusFalseNodes,
	                                    bool              doWriteFHiCL,
	                                    size_t            maxFragmentSizeBytes,
	                                    size_t            routingTimeoutMs,
	                                    size_t            routingRetryCount);
	static void extractEventBuildersInfo(ConfigurationTree artdaqSupervisorNode,
	                                     bool              getStatusFalseNodes,
	                                     bool              doWriteFHiCL,
	                                     size_t            maxFragmentSizeBytes);
	static void extractDataLoggersInfo(ConfigurationTree artdaqSupervisorNode,
	                                   bool              getStatusFalseNodes,
	                                   bool              doWriteFHiCL,
	                                   size_t            maxFragmentSizeBytes);
	static void extractDispatchersInfo(ConfigurationTree artdaqSupervisorNode,
	                                   bool              getStatusFalseNodes,
	                                   bool              doWriteFHiCL,
	                                   size_t            maxFragmentSizeBytes);
  ...
  protected:
	std::map<ARTDAQAppType, std::map<std::string /*name*/, std::string /*fcl*/>>
	    fclMap_;  //cache for configure blob
  ...
  public:
	static struct ProcessTypes
	{
		...
		ProcessTypes()
		    : mapToType_(
		          {std::make_pair(READER, ARTDAQTableBase::ARTDAQAppType::BoardReader),
		           ...
		           std::make_pair(ROUTER,
		                          ARTDAQTableBase::ARTDAQAppType::RoutingManager)})
		    , mapToTable_(
		    ...
		    , mapToLinkGroupIDColumn_(
		          {std::make_pair(READER,
		                          ARTDAQTableBase::colARTDAQSupervisor_
		                              .colLinkToBoardReadersGroupID_),
		           std::make_pair(BUILDER,
		                          ARTDAQTableBase::colARTDAQSupervisor_
		                              .colLinkToEventBuildersGroupID_),
		           std::make_pair(LOGGER,
		                          ARTDAQTableBase::colARTDAQSupervisor_
		                              .colLinkToDataLoggersGroupID_),
		           std::make_pair(DISPATCHER,
		                          ARTDAQTableBase::colARTDAQSupervisor_
		                              .colLinkToDispatchersGroupID_),
		           std::make_pair(ROUTER,
		                          ARTDAQTableBase::colARTDAQSupervisor_
		                              .colLinkToRoutingManagersGroupID_)})
		    , mapToGroupIDColumn_({std::make_pair(READER, "BoardReaderGroupID"),
		...
		const std::map<std::string /*processType*/, ARTDAQTableBase::ARTDAQAppType>
		    mapToType_;
		const std::map<std::string /*processType*/, std::string /*typeTable*/>
		    mapToTable_, mapToGroupIDAppend_, mapToLinkGroupIDColumn_,
		    mapToGroupIDColumn_;
	} processTypes_;
