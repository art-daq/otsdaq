	...
	const int64_t                         availableDataSpaceKB)
{
	...
	SupervisorInfo::emplaceAvailableSpace(availableLogSpaceKB, subapps_.at(name).availableLogSpaceKB_);
	SupervisorInfo::emplaceAvailableSpace(availableDataSpaceKB, subapps_.at(name).availableDataSpaceKB_);
	if(status !=
	   SupervisorInfo::
	       APP_STATUS_UNKNOWN)  // if unknown, then do not consider it a status update
...
void SupervisorInfo::emplaceAvailableSpace(const int64_t availableSpaceKB, 
	std::deque<std::pair<time_t, int64_t>>& availableSpaceDeque)
{
	if(availableSpaceKB > 0) //only insert valid values
	{		
		__GEN_COUTTV__(availableSpaceKB);
		time_t now = time(0);
		...
		if(TTEST(1))
		{
			...
			for(auto& val : availableSpaceDeque)
			{
				ss << i << ":(t=" << val.first << ", KBs=" << val.second << 
					", dt=" << now - val.first << 
					" < " << ((1 << (i/2))*225 + (1 << (i/2))*225/2) <<
					", KBps=" << (val.second - availableSpaceKB)*1.0f/(1 + now - val.first) <<
					") ";
				++i;
			}
			__COUTT__ << mfSubject_ << " " << 
				&availableSpaceDeque << " " << ss.str() << __E__ <<
				getLogUsageRateLastHourKBps() << " KB/s";
		}
	}
