...
ots::TFMSupervisorTRACEController::TFMSupervisorTRACEController() {}

const ots::ITRACEController::HostTraceLevelMap& ots::TFMSupervisorTRACEController::getTraceLevels()
{
	...
	if(theSupervisor_)
	{
		...
		for(auto& comm : commanders)
		{
			if(traceLevelsMap_.count(comm.first.host) == 0)
			{
				...
				auto lvls      = TFMSupervisor::tokenize_(lvlstring);
...
void ots::TFMSupervisorTRACEController::setTraceLevelMask(const std::string& label,
