...
class SlowControlsVInterface : public Configurable, public VStateMachine
{
  public:
	...
	virtual std::vector<std::vector<std::string>> getChannelHistory(
	    const std::string& Name, int startTime, int endTime) = 0;
	virtual std::vector<std::vector<std::string>> getLastAlarms(
	    const std::string& pvName) = 0;
	virtual std::vector<std::vector<std::string>> getAlarmsLog(
	    const std::string& pvName)                                              = 0;
	...
	void setNewAlarmCallback(std::function<void()> callback)
	{
		newAlarmCallback_ = callback;
	}
