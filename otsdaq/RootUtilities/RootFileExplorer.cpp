...
//================================================================================================
RootFileExplorer::RootFileExplorer(std::string fSystemPath,
...
xercesc::DOMDocument* RootFileExplorer::initialize(bool liveDQMFlag)
{
	...
	theImplementation_ = xercesc::DOMImplementationRegistry::getDOMImplementation(
	...
	if(theImplementation_)
	{
		...
		{
			STDLINE(
			    string("Error Message: ") + xercesc::XMLString::transcode(e.getMessage()),
			    ACRed);
		}
		catch(...)
		{
	...
	if(liveDQMFlag_)
	{
		...
		ss_.str("");
		ss_ << "Main anchor (document): " << rootElement_;
	...
	{
		string fName = fSystemPath_ + string("/") + fRootPath_;
		if(fFoldersPath_.length() > 0)
			fName += string("/") + fFoldersPath_;
		if(fFileName_.length() > 0)
			fName += string("/") + fFileName_;
...
//==========================================================================================
xercesc::DOMElement* RootFileExplorer::populateBinaryTreeNode(
...
{
	ss_.str("");
	ss_ << "fRFoldersPath_: " << fRFoldersPath_;
	STDLINE(ss_.str(), ACGreen);
	ss_.str("");
	ss_ << "previous      : " << previousAncestor_;
	STDLINE(ss_.str(), ACGreen);
	...
	if(theNodes_.find(previousAncestor_) == theNodes_.end())  // a new node
	{
		...
		ss_.str("");
		ss_ << "NEW nodes (" << nodes << ") for name " << name;
		STDLINE(ss_.str(), "");
	...
	else  // Is already there
	{
		...
		ss_.str("");
		ss_ << name << " points to an OLD nodes (" << nodes << ") parallel to "
		    << theNodeName_[previousAncestor_];
		STDLINE(ss_.str(), "");
	}
	...
	ss_.str("");
	ss_ << "Attaching node " << node << " previousAncestor_ " << previousAncestor_
	    << " to nodes " << nodes;
	STDLINE(ss_.str(), "");

	xercesc::DOMElement* ancestor =
	    theDocument_->createElement(xercesc::XMLString::transcode("ancestor"));
	xercesc::DOMText* ancestorVal = theDocument_->createTextNode(
	    xercesc::XMLString::transcode(previousAncestor_.c_str()));
	ancestor->appendChild(ancestorVal);
	node->appendChild(ancestor);

	xercesc::DOMElement* fSystemPath =
	...
	fRFoldersPath =
	    theDocument_->createElement(xercesc::XMLString::transcode("fRFoldersPath"));
	...
	fFileNameVal =
	    theDocument_->createTextNode(xercesc::XMLString::transcode(fFileName_.c_str()));
	fHistNameVal =
	    theDocument_->createTextNode(xercesc::XMLString::transcode(fHistName_.c_str()));
	fRFoldersPathVal = theDocument_->createTextNode(
	    xercesc::XMLString::transcode(fRFoldersPath_.c_str()));

	if(isLeaf)
	{
		fDisplayNameVal = theDocument_->createTextNode(
		    xercesc::XMLString::transcode(fHistName_.c_str()));
	}
	else
	{
		fDisplayNameVal =
                                             ...
                                             xercesc::DOMElement* anchorNode)
{
	...
	if(currentDir != 0)
	{
		...
		while((obj = (TObject*)nextobj()))
		{
			...
			ss_.str("");
			ss_ << "Exploring " << objName << " class: " << string(obj->ClassName());
			STDLINE(ss_.str(), "");
			if(string(obj->ClassName()) == "TTree")
				continue;
			if(string(obj->ClassName()) == "TNtuple")
				continue;
			if(string(obj->ClassName()) == "TGeoManager")
				continue;
			if(string(obj->ClassName()) == "TGeoVolume")
				continue;
			if(obj->IsFolder())
			{
				ss_.str("");
				...
				anchorNodeLast_   = this->populateBinaryTreeNode(
                    anchorNode, string(obj->GetName()), false);
			...
			else  // is TH1 or TCanvas
			{
				...
				previousAncestor_ = currentDir->GetName();
				anchorNode =
				    this->populateBinaryTreeNode(anchorNodeLast_, fHistName_, true);
			}
		}
	}
	else
                                               ...
                                               xercesc::DOMElement* anchorNode)
{
	...
	while((keyH = (TKey*)hList()))
	{
		...
		ss_.str("");
		ss_ << "Exploring " << objName << " level: " << level
		    << " COUNTER: " << ++counter_;
		///        STDLINE(ss_.str(),"") ;
...
{
	ss_.str("");
	ss_ << what << " - hierarchyPaths_.size(): " << hierarchyPaths_.size();
	STDLINE(ss_.str(), "");
	for(int i = 0; i < (int)hierarchyPaths_.size(); i++)
	{
		ss_.str("");
		ss_ << i << "] " << hierarchyPaths_[i];
		STDLINE(ss_.str(), "");
	}
}
//================================================================================================
std::string RootFileExplorer::computeHierarchyPaths(void)
{
	...
	for(int i = 0; i < (int)hierarchyPaths_.size(); i++)
	{
		...
		if(i != (int)hierarchyPaths_.size() - 1)
			name += string("/");
	}
	return name;
}
...
void RootFileExplorer::initializeXMLWriter(void)
{
	...
	if(theSerializer_->getDomConfig()->canSetParameter(
	       XMLUni::fgDOMWRTDiscardDefaultContent, true))
		theSerializer_->getDomConfig()->setParameter(
		    XMLUni::fgDOMWRTDiscardDefaultContent, true);

	if(theSerializer_->getDomConfig()->canSetParameter(XMLUni::fgDOMWRTFormatPrettyPrint,
	                                                   true))
		theSerializer_->getDomConfig()->setParameter(XMLUni::fgDOMWRTFormatPrettyPrint,
		                                             true);
