...
try
{
	...
	if(filterString == "")
		result = ifc.findGlobalConfigurations("*");  // GConfig will return all GConfig* with filesystem db.. for mongodb would require reg expr
	else
		result = ifc.findGlobalConfigurations(filterString + "*");  // GConfig will return all GConfig* with filesystem db.. for mongodb would// mongodb would require reg expr
	auto end = std::chrono::high_resolution_clock::now();
	auto duration =
	    std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
	__COUTT__ << "Time taken to call "
...
try
{
	...
	std::set<std::string> returnSet = ifc.findGlobalConfigurationsContaining(tableName, version.toString());
