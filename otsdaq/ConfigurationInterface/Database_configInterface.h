...
class DatabaseConfigurationInterface : public ConfigurationInterface
{
  public:
	...
	std::set<std::string /*name+version*/> getAllTableGroupNames(
	    std::string const& filterString = "") const override;
	std::set<TableGroupKey>         getKeys(const std::string& groupName) const override;
	std::set<std::string /*group*/> findGroupsWithTable(
	    std::string const& tableName, TableVersion version) const override;

	TableGroupKey findLatestGroupKey(
	    const std::string& groupName) const noexcept override;

	/// return the contents of a table group
	table_version_map_t getTableGroupMembers(
	    std::string const& tableGroup, bool includeMetaDataTable = false) const override;

	/// create a new table group from the contents map
	void saveTableGroup(table_version_map_t const& memberMap,
	                    std::string const&         tableGroup) const override;

	std::pair<std::string, TableVersion> saveCustomJSON(
	    const std::string& JSON, const std::string& documentNameToSave) const override;
	std::string loadCustomJSON(const std::string& documentNameToLoad,
	                           TableVersion       documentVersionToLoad) const override;

  private:
	table_version_map_t getCachedTableGroupMembers(std::string const& tableGroup) const;
	void                saveTableGroupMemberCache(table_version_map_t const& memberMap,
