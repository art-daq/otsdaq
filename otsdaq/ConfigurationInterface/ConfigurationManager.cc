                                             ...
                                             bool               onlyDeactivate)
{
	...
	getActiveContextMemberNames();  // make sure active contextMemberNames_ are up to date
    ...
    std::ostream&      altOut /* = std::cout */)
{
	...
	auto localDumpActiveGroupMembers = [](ConfigurationManager* cfgMgr,
		...
		for(it = activeGroups.begin(); it != activeGroups.end(); ++it)
		{
			...
			cfgMgr->loadTableGroup(it->second.first,
			                       it->second.second,
			                       false /*doActivate*/,
			                       &memberMap /*memberMap*/,
			                       0 /*progressBar*/,
	...
	auto localDumpActiveTableContents = [](const ConfigurationManager* cfgMgr,
		...
		for(it = activeTables.begin(); it != activeTables.end(); ++it)
		{
			if(jsonify)
			{
				__COUTT__ << "localDumpActiveTableContents table: " << it->first << __E__;
...
/// 	if accumulatedWarnings, then "ignore warnings" and return them in the string
    ...
    std::map<std::string /* tableName */, TableVersion> overrideTables /*={}*/)
{
	...
	try
	{
		...
		try
		{
			...
			if(accumulatedWarnings)
			{
				__GEN_COUTT__ << "Checking chosen group for tree errors... here are the "
				                 "current warnings: "
				...
				__GEN_COUTT__ << "After checking children warnings: "
				              << *accumulatedWarnings << __E__;
			}
...
catch(...)
{
	...
	{
		ss << " Run-time Exception message: " << e.what();
	}
	catch(const std::exception& e)
	{
		ss << " Exception message: " << e.what();
	}
	__COUT_WARN__ << ss.str() << __E__;
...
TableBase* ConfigurationManager::getVersionedTableByName(
...
//==============================================================================
/// getVersionAliases()
///	get version aliases with table alias that matches tableAliasNeedle, for currently active backbone tables
...
std::map<std::string, TableVersion> ConfigurationManager::getActiveVersions(void) const
{
	...
	{
		__GEN_COUTS__(2) << table.first << __E__;
		...
		if(username != "")  //has username
		{
			cmdResult = StringMacros::exec(
			    ("rm " + tmpSubsystemFilename +
			     " 2>/dev/null; scp -o ConnectTimeout=5 -o BatchMode=yes " + username +
			...
			cmdResult = StringMacros::exec(
			    ("rm " + tmpSubsystemFilename +
			     " 2>/dev/null; scp -o ConnectTimeout=5 -o BatchMode=yes " + hostname +
