    ...
    ConfigurationTree::BitMap<std::string>& bitmap) const
{
	   ...
	   col_ != TableView::INVALID)  // this node is a value node
	{
		...
		// extract bit map
		{
			...
			for(unsigned int i = 0; i < bitmapString.length(); i++)
			{
				...
				else if(startInt == (unsigned int)-1)  // need to find start of number
				{
					...
					else if((bitmapString[i] == '-') || (bitmapString[i] == '+') ||
					        (bitmapString[i] >= '0' &&
					   ...
					   ']')  // found end of row, assume row and number ended
					{
						...
						if(bmp.mapsToStrings_)  //convert string to number (since this template function is assumed to handle numbers only; i.e., it is not the special strings version)
						{
							...
							startInt = -1;
						        ...
						        ',')  // comma found, assume end of number
						{
							//drop space or end quote
							unsigned int ii          = i;
							bool         wasEndQuote = false;
							...
							if(bmp.mapsToStrings_)  //convert string to number (since this template function is assumed to handle numbers only; i.e., it is not the special strings version)
							{
								...
								catch(const std::out_of_range& e)
								{
									...
									if(!wasEndQuote)
									{
										...
										try
										{
											index = std::stoul(bitmapString.substr(
											    startInt, ii - startInt));
										}
										catch(...)
										{
											ss << "Interpreting value as integer index "
											      "failed for '"
											   << bitmapString.substr(startInt,
											                          ii - startInt)
											   << "'." << __E__;
											__SS_THROW__;
										}
				...
				{
					__SS__ << "Illegal mismatch in number of rows and columns. Extracted "
					          "data was "
		...
		/// getValueAsBitMap = getValue (only ConfigurationTree::BitMap value)
		///
		///	Special string version of getValue for ConfigurationTree::BitMap type
		...
		const std::string& ConfigurationTree::getTableName(void) const
		{
			if(!table_)
			{
				__SS__ << "Can not get configuration name of node with no configuration "
				          "pointer! "
				       << "Is there a broken link? " << __E__;
				if(linkParentTable_)
				{
					ss << "Error occurred traversing from "
					   << linkParentTable_->getTableName() << " UID '"
					   << linkParentTable_->getView().getValueAsString(
					          linkBackRow_, linkParentTable_->getView().getColUID())
					   << "' at row " << linkBackRow_ << " col '"
					   << linkParentTable_->getView()
		...
		const std::string& ConfigurationTree::getParentRecordName(void) const
		{
			...
			__SS__ << "Can not get parent record name of node without the parent table "
			          "pointer "
			       << (linkParentTable_ ? "" : "= null ")
		...
		const std::string& ConfigurationTree::getParentLinkColumnName(void) const
		{
			...
			__SS__ << "Can not get parent link column name of node without the parent "
			          "table pointer "
			       << (linkParentTable_ ? "" : "= null ")
			       << "and row (row = " << linkBackRow_
		...
		std::string ConfigurationTree::getParentLinkIndex(void) const
		{
			...
			__SS__ << "Can not get parent link index of node without the parent table "
			          "pointer "
			       << (linkParentTable_ ? "" : "= null ")
			       << "and row (row = " << linkBackRow_
			       << ") and col (col = " << linkBackCol_
		...
		const std::string& ConfigurationTree::getFieldTableName(void) const
		{
			...
			if(isLinkNode())
			{
				if(!linkParentTable_)
				{
					__SS__ << "Can not get configuration name of link node field with no "
					          "parent "
		...
		const std::string& ConfigurationTree::getDisconnectedTableName(void) const
		{
			...
			__SS__ << "Can not get disconnected target name of node unless it is a "
			          "disconnected "
			          "link node!"
			       << __E__;
		...
		const TableVersion& ConfigurationTree::getTableVersion(void) const
		{
			if(!tableView_)
			{
				__SS__ << "Can not get configuration version of node with no config view "
				          "pointer!"
				       << __E__;
		...
		const time_t& ConfigurationTree::getTableCreationTime(void) const
		{
			if(!tableView_)
			{
				__SS__ << "Can not get configuration creation time of node with no "
				          "config view "
				          "pointer!"
				       << __E__;
		...
		std::vector<std::string> ConfigurationTree::getFixedChoices(void) const
		{
			...
			if(isLinkNode())
			{
				if(!linkParentTable_)
				{
					__SS__ << "Can not get fixed choices of node with no parent config "
					          "view pointer!"
					       << __E__;
				...
				std::vector<std::string> choices =
				    parentView->getColumnInfo(c).getDataChoices();
			...
			std::vector<std::string> choices =
			    tableView_->getColumnInfo(col_).getDataChoices();
			for(const auto& choice : choices)
				retVec.push_back(choice);
		...
		const std::string& ConfigurationTree::getValueAsString(bool returnLinkTableValue)
		    const
		{
			...
			if(isLinkNode())
			{
				...
				return tableView_->getDataView()[row_][col_];
			        ...
			        col_ ==
			            TableView::INVALID)  // this node is table node maybe with groupId
			{
				...
				if(getValueType() == TableViewColumnInfo::TYPE_ON_OFF ||
				   ...
				   getValueType() == TableViewColumnInfo::TYPE_YES_NO)
					return getValueAsString() ==
					       TableViewColumnInfo::
					           DATATYPE_BOOL_DEFAULT;  // default to OFF, NO,
					                                   // FALSE
		...
		ConfigurationTree ConfigurationTree::recurse(
		    const ConfigurationTree& tree,
		    const std::string&       childPath,
		    bool                     doNotThrowOnBrokenUIDLinks,
		    const std::string&       originalNodeString)
		{
			__COUTS__(50) << tree.row_ << " " << tree.col_ << __E__;
			__COUTS__(51) << "childPath=" << childPath << " " << childPath.length()
			              << __E__;
		...
		ConfigurationTree ConfigurationTree::getNode(
		    const std::string& nodeString, bool doNotThrowOnBrokenUIDLinks) const
		{
		    ...
		    const std::string& originalNodeString) const
		{
			...
			{
				__SS__ << "Invalid empty node name! Looking for child node '"
				       << nodeString << "' from node '" << getValue() << "'..." << __E__;
			...
			std::string nodeName =
			    nodeString.substr(startingIndex, endingIndex - startingIndex);
			__COUTS__(51) << "nodeName=" << nodeName << " len=" << nodeName.length()
			              << __E__;
			...
			try
			{
				__COUTS__(50) << row_ << " " << col_ << " " << groupId_ << " "
				              << tableView_ << __E__;
				if(isRootNode())
				{
					// root node
				...
				else if(row_ == TableView::INVALID && col_ == TableView::INVALID)
				{
					...
					if(!tableView_)
					{
						__SS__ << "Missing configView pointer! Likely attempting to "
						          "access a "
				...
				else if(col_ == TableView::INVALID)
				{
					...
					__COUTS__(51)
					    << "nodeName=" << nodeName << " " << nodeName.length() << __E__;
					   ...
					   !isGroupLink)
					{
						...
						try
						{
							...
							childConfig
							    ->getView();  // get view as a test for an active view
						...
						catch(...)
						{
							__COUTS__(50)
							    ...
							    << " at entry with UID "
							    << tableView_
							           ->getDataView()[row_][tableView_->getColUID()]
							    << __E__;
							...
							return ConfigurationTree(
							    ...
							    nodeName,
							    tableView_
							        ->getDataView()[row_][c],  // this the link node field
					...
					else if(isLink)
					{
						...
						catch(...)
						{
							...
							return ConfigurationTree(
							    ...
							    0,
							    tableView_
							        ->getDataView()[row_][linkPair.second],  // groupID
							    table_,  // linkParentTable_
							    nodeName,
							    tableView_
							        ->getDataView()[row_][c],  // this the link node field
						...
						return recurse(
						    ConfigurationTree(  // this is a link node
						        ...
						        tableView_
						            ->getDataView()[row_]
						                           [linkPair.second],  // groupId string
						        table_,                                // linkParentTable_
						        nodeName,  // this is a link node
						        tableView_
						            ->getDataView()[row_][c],  // this the link node field
			...
			catch(std::runtime_error& e)
			{
				__SS__ << "\n\nError occurred descending from node '" << getValue()
				       << "' in table '" << getTableName() << "' looking for child '"
				       << nodeName << "'\n\n"
				       << __E__;
			...
			__SS__ << "\n\nError occurred descending from node '" << getValue()
			       << "' in table '" << getTableName() << "' looking for child '"
			       << nodeName << "'\n\n"
			       << "Invalid depth! getNode() called from a value point in the "
			          "Configuration Tree."
			       << __E__;
			ss << "The original node search string was '" << originalNodeString << ".'"
			   << __E__;
		...
		std::string ConfigurationTree::nodeDump(bool forcePrintout) const
		{
			...
			{
				ss << "\t"
				   << "Node dump initiated from node '" << getValueAsString() << "'..."
				   << __E__;
			}
			catch(...)
			{
			...
			try
			{
				...
				if(isTableNode() || isGroupLinkNode())
				{
					...
					ss << "\t"
					   << "Here is the list of possible children (count = "
					   << children.size() << "):" << __E__;
		...
		//==============================================================================
		ConfigurationTree ConfigurationTree::getForwardNode(
		    std::string nodeName, unsigned int forwardSteps) const
		{
			unsigned int s = 0;
		    ...
		    bool autoSelectFilterFields) const
		{
			...
			std::vector<int>
			    fieldCount;  //-1 := guaranteed, else count must match num of records
			...
			if(autoSelectFilterFields)
			{
				...
				std::set<std::pair<unsigned int /*fieldPriority*/,
				...
				{
					__COUTS__(11)
					    << "Option [" << i << "] " << fieldCandidateList[i].relativePath_
					    << fieldCandidateList[i].columnName_ << " : "
					    << fieldCandidateList[i].columnInfo_->getType() << ":"
					    << fieldCandidateList[i].columnInfo_->getDataType() << __E__;

					priorityPenalty =
					    std::count(fieldCandidateList[i].relativePath_.begin(),
					...
					prioritySet.emplace(std::make_pair(priorityPenalty /*fieldPriority*/,
					                                   i /*fieldIndex*/));
					__COUTS__(11)
					    << "Option [" << i << "] " << fieldCandidateList[i].relativePath_
					    << fieldCandidateList[i].columnName_ << " : "
					...
					for(const auto& priorityFieldIndex : prioritySet)
						if(++cnt > 3)  // then mark for erasing
						{
							__COUTS__(11) << cnt << " marking "
							              << fieldCandidateList[priorityFieldIndex.second]
							                     .relativePath_
							              << fieldCandidateList[priorityFieldIndex.second]
							                     .columnName_
							              << __E__;
			...
			for(unsigned int i = 0; i < recordList.size(); ++i)
			{
				...
				if(node.isGroupIDNode())
				{
					...
					if(i == 0 && fieldGroupIDChildLinkIndex)
						*fieldGroupIDChildLinkIndex =
						    node.getColumnInfo().getChildLinkIndex();
	...
	//	=====================
			...
			for(const auto& fieldNode : recordChildren)
			{
				...
				if(fieldNode.second.isValueNode() || fieldNode.second.isGroupLinkNode())
				{
					...
					if(inFirstRecord)  // first uid record
					{
						...
						// check field accept filter list
						found =
						    fieldAcceptList.size() ? false : true;  // accept if no filter
						                                            // list
						for(const auto& fieldFilter : fieldAcceptList)
							...
							else if(StringMacros::wildCardMatch(fieldFilter,
							                                    fieldNode.first))
							{
								found = true;
								break;
							    ...
							    fieldCandidateList[j].columnName_))
							{
								...
								if(fieldNode.second.isGroupLinkNode() &&
								   j + 1 < fieldCandidateList.size())
									++fieldCount
									    [j + 1];  // increment associated link index too!
								break;
							}
						}
					}
					...
					else  // not first record
					{
						...
						for(j = 0; j < fieldCandidateList.size() - 1; ++j)
						{
							    ...
							    fieldCandidateList[j].columnName_))
							{
								...
								++fieldCount[j];
								++fieldCount[j +
								             1];  // increment associated link index too!
								break;
							}
						}
					}
		...
		ConfigurationTree::getChildrenNamesByPriority(bool onlyStatusTrue) const
		{
			...
			if(!tableView_)
			{
				...
				if(isLinkNode() && isDisconnected())
					ss << " This node is a disconnected link to "
					   << getDisconnectedTableName() << __E__;
			...
			else if(col_ == TableView::INVALID)
			{
				...
				for(unsigned int c = 0; c < tableView_->getNumberOfColumns(); ++c)
					if(c ==
					       tableView_
					           ->getColUID() ||  // skip UID and linkID columns (only show
			...
			else  // this node is value node, so has no node to choose from
			{
				...
				__SS__ << "\n\nError occurred looking for children of nodeName="
				       << getValueName() << "\n\n"
				       << "Invalid depth! getChildrenValues() called from a value point "
				          "in the "
				          "Configuration Tree."
				       << __E__;
		    ...
		    bool byPriority /* = false */, bool onlyStatusTrue /* = false */) const
		{
			...
			if(!tableView_)
			{
				...
				if(isLinkNode() && isDisconnected())
					ss << " This node is a disconnected link to "
					   << getDisconnectedTableName() << "(" << getDisconnectedLinkID()
					   << ")" << __E__;
				__SS_ONLY_THROW__;
			}
		...
		ConfigurationTree ConfigurationTree::getValueAsTreeNode(void) const
		{
			...
			std::string valueString = tableView_->getValueAsString(
			    row_, col_, true /* convertEnvironmentVariables */);
