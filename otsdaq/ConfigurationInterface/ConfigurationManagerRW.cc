...
{
	__GEN_COUT__ << "Instantiating Config Manager with Write Access! (for " << username
	             << ") time=" << time(0) << " runTimeSeconds()=" << runTimeSeconds()
	             << __E__;
			...
			if(fp)
			{
				...
				if(!foundVector[i])
					fprintf(
					    fp,
					    "\nContextGroup/%s",
					    ConfigurationManager::CONTEXT_SUBSYSTEM_OPTIONAL_TABLE.c_str());
    ...
    bool               initializeActiveGroups /* = false */)
{
	...
	__GEN_COUT__ << "======================================================== "
	                "getAllTableInfo start runTimeSeconds()="
	             << runTimeSeconds() << __E__;
	{
			...
			if((pDIR = opendir(path.c_str())) != 0)
			{
				while((entry = readdir(pDIR)) != 0)
				{
					...
					catch(cet::exception const&)
					{
						...
						__GEN_COUT__ << "Skipping! No valid class found for... "
						             << entry->d_name << "\n";
						continue;
					}
					catch(std::runtime_error& e)
					{
						...
						if(accumulatedWarnings)
						{
							...
							__GEN_COUT__ << "Attempting to allow illegal columns!"
							             << __E__;
							...
							{
								__GEN_COUT__ << "Skipping! Allowing illegal columns "
								                "didn't work either... "
								             << entry->d_name << "\n";
								continue;
							}
						...
						for(auto& version : versions)
							if(version.isTemporaryVersion())
							{
								try  // do NOT let TableView::init() throw here
								{
								    ...
								    ...)  // do NOT let invalid temporary version throw at this
								          // point
								{
					...
					allTableInfo_[entry->d_name].versions_ =
					    theInterface_->getVersions(table);
		...
		else  //multi-threading
		{
			...
			if((pDIR = opendir(path.c_str())) != 0)
			{
				while((entry = readdir(pDIR)) != 0)
				{
					...
					tableName =
					    entry
					        ->d_name;  //copy immediate, thread does not like using pointer to char*, corrupts immediately on wraparound

					//make temporary table info for thread
					sharedTableInfoPtrs.push_back(std::make_shared<ots::TableInfo>());
					sharedTableInfoPtrs.back()->accumulatedWarnings_ =
					    accumulatedWarnings ? "ALLOW"
					                        : "";  //mark to allow accumulated warnings
					...
					__GEN_COUTT__ << "Starting thread... " << foundThreadIndex
					...
					std::thread(
					    [](ConfigurationManagerRW*            theCfgMgr,
					       ...
					       std::shared_ptr<std::atomic<bool>> theThreadDone) {
						    ConfigurationManagerRW::loadTableInfoThread(theCfgMgr,
						                                                theTableName,
						                                                existingTable,
						                                                theTableInfo,
						                                                theThreadDone);
			...
			for(auto& tableInfo : sharedTableInfoPtrs)
			{
				...
				nameToTableMap_[tableInfo->tablePtr_->getTableName()] =
				    tableInfo->tablePtr_;
				allTableInfo_[tableInfo->tablePtr_->getTableName()].tablePtr_ =
				    tableInfo->tablePtr_;
				allTableInfo_[tableInfo->tablePtr_->getTableName()].versions_ =
				    tableInfo->versions_;
	...
	__GEN_COUT__ << "======================================================== "
	                "getAllTableInfo end runTimeSeconds()="
	             << runTimeSeconds() << __E__;
	...
	if(getGroupKeys || getGroupInfo)
	{
		...
		try
		{
			...
			__GEN_COUTT__ << "Group Info start runTimeSeconds()=" << runTimeSeconds()
			              << __E__;
			...
			if(getGroupInfo)
			{
				...
				if(numOfThreads < 2)  // no multi-threading
					for(auto& groupInfo : allGroupInfo_)
					{
						try
						{
							groupInfo.second.latestKey_ = groupInfo.second.getLastKey();
							loadTableGroup(
						...
						catch(...)
						{
							...
							groupInfo.second.latestKeyGroupAuthor_ =
							    ConfigurationManager::UNKNOWN_INFO;
							groupInfo.second.latestKeyGroupCreationTime_ =
							    ConfigurationManager::UNKNOWN_TIME;
							groupInfo.second.latestKeyGroupTypeString_ =
				...
				else   //multi-threading
				{
					...
					for(auto& groupInfo : allGroupInfo_)
					{
						...
						__GEN_COUTT__ << "Starting thread... " << foundThreadIndex
						              << " for " << groupInfo.first << "("
						              << groupInfo.second.getLastKey() << ")" << __E__;
						...
						*(threadDone[foundThreadIndex]) = false;
						...
						std::thread(
						    [](ConfigurationManagerRW*            theCfgMgr,
						       ...
						       std::shared_ptr<std::atomic<bool>> theThreadDone) {
							    ConfigurationManagerRW::loadTableGroupThread(
							        theCfgMgr,
							        theGroupName,
							        theGroupKey,
							        theGroupInfo,
					...
					for(auto& groupInfo : allGroupInfo_)
					{
						groupInfo.second.latestKey_ = sharedGroupInfoPtrs[i]->latestKey_;
						groupInfo.second.latestKeyGroupComment_ =
						    sharedGroupInfoPtrs[i]->latestKeyGroupComment_;
						groupInfo.second.latestKeyGroupAuthor_ =
						    sharedGroupInfoPtrs[i]->latestKeyGroupAuthor_;
						groupInfo.second.latestKeyGroupCreationTime_ =
						    sharedGroupInfoPtrs[i]->latestKeyGroupCreationTime_;
						groupInfo.second.latestKeyGroupTypeString_ =
						    sharedGroupInfoPtrs[i]->latestKeyGroupTypeString_;
						groupInfo.second.latestKeyMemberMap_ =
						    sharedGroupInfoPtrs[i]->latestKeyMemberMap_;
						++i;
					}  //end copy group info loop
		...
		{
			__SS__ << "An unknown fatal error occurred reading the info for all table "
			          "groups."
			       << __E__;
			try
			{
...
try
{
	...
	try  // only add valid table instances to maps
	{
		cfgMgr->theInterface_->get(tableInfo->tablePtr_,
		                           tableName,
		                           0,
		                           0,
		                           true);  // dont fill
	...
	catch(std::runtime_error& e)
	{
		...
		if(tableInfo->accumulatedWarnings_ == "ALLOW")
		{
			...
			{
				tableInfo->tablePtr_ =
				    new TableBase(tableName, &returnedAccumulatedErrors);
			}
			catch(...)
			{
			...
			__COUT_WARN__ << "Error (but allowed): " << returnedAccumulatedErrors
			              << __E__;
	...
	{
		__COUTT__ << "Copying temporary version from existing table object for "
		          << tableName << __E__;
		// copy the existing temporary versions! (or else all is lost)
...
void ConfigurationManagerRW::loadTableGroup(
...
try
{
	__COUTT__ << "Thread started... " << groupName << "(" << groupKey << ")" << __E__;

	groupInfo->latestKey_ = groupKey;
...
try
{
	...
	std::map<std::string /*name*/, std::string /*alias*/>*
	    compareToMemberTableAliasesPtr = nullptr;
	if(memberTableAliases
	       .size())  //only give pointer if necessary, without will load group faster
	...
	for(auto& memberPair : groupMemberMap)
	{
		...
		if(memberTableAliases.find(memberPair.first) != memberTableAliases.end())
		{
			...
			if(compareToMemberTableAliases.find(memberPair.first) ==
			       compareToMemberTableAliases.end() ||  // alias is missing
			   memberTableAliases.at(memberPair.first) !=
...
{
	__COUT_WARN__ << "Error occurred comparing group '" << groupName << "("
	              << groupKeyToCompare << ")'..." << __E__;
                                                ...
                                                std::string*       groupTypeString)
{
	...
	{
		__GEN_COUT_ERR__ << "There were errors, so de-activating group: "
		                 << tableGroupName << " (" << tableGroupKey << ")" << __E__;
	...
	__GEN_COUT_INFO__ << "Updating persistent active groups to "
	...
	// save last activated group
	{
		std::pair<std::string /*group name*/, TableGroupKey> activatedGroup(
		    std::string(tableGroupName), tableGroupKey);
...
{
	__GEN_COUT_INFO__ << "Creating temporary backbone view from version "
	                  << sourceViewVersion << __E__;
...
TableBase* ConfigurationManagerRW::getTableByName(const std::string& tableName)
{
	if(nameToTableMap_.find(tableName) == nameToTableMap_.end())
	{
		...
		ss << "\nIf you think this table should exist in the core set of tables, try "
		      "running 'UpdateOTS.sh --tables' to update your tables, then relaunch ots."
		   << __E__;
		ss << "\nTables must be defined at path $USER_DATA/TableInfo/ to exist in ots. "
		      "Please verify your table definitions, and then restart ots."
		   << __E__;
		__GEN_COUT_ERR__ << "\n" << ss.str();
		__SS_THROW__;
	}
...
void ConfigurationManagerRW::preloadVersionCreationTimes(void)
{
	...
	{
		size_t maxVersions = 0;
		for(const auto& gw : groupedWork)
			if(gw.second.size() > maxVersions)
				maxVersions = gw.second.size();
		for(size_t vi = 0; vi < maxVersions; ++vi)
			for(const auto& gw : groupedWork)
				if(vi < gw.second.size())
					flatWork.emplace_back(gw.first, gw.second[vi]);
	}
	...
	if(numOfThreads < 2)
	{
		for(const auto& work : flatWork)
		{
			try
			{
				getVersionCreationTime(work.first, work.second);
			}
			catch(...)
			{
	...
	else
	{
		...
		auto*                    iface = theInterface_;

		for(int i = 0; i < numOfThreads; ++i)
			threads.emplace_back([iface, &workIndex, &flatWork, this]() {
				std::map<std::string, TableBase*> localTables;

				try
				{
					size_t w;
					while((w = workIndex++) < flatWork.size())
					{
						const auto& tableName = flatWork[w].first;
						const auto& version   = flatWork[w].second;
						...
						try
						{
							...
							time_t creationTime =
							    table->getView(version).getCreationTime();

							{
								std::lock_guard<std::mutex> lock(
								    versionCreationTimeCacheMutex_);
								versionCreationTimeCache_[tableName][version] =
								    creationTime;
							}
						}
						catch(...)
                                                   ...
                                                   TableVersion       targetVersion)
{
	...
	if(targetVersion.isInvalid())
	{
		    ...
		    /*no increment*/)
		{
			...
			{
				__GEN_COUT__ << "Removing '" << tableName << "' version info: " << *it
				             << __E__;
	...
	else  // erase target version only
	{
		...
		{
			__GEN_COUT__ << "Target '" << tableName << "' version v" << targetVersion
    ...
    const std::map<std::string /*name*/, std::string /*alias*/>& memberTableAliases)
{
	...
	else  //multi-threading
	{
			...
			*(threadDone[foundThreadIndex]) = false;
			...
			std::thread(
			    [](ConfigurationManagerRW*                    cfgMgr,
			       ...
			       ots::TableGroupKey                         groupKeyToCompare,
			       const std::map<std::string, TableVersion>& groupMemberMap,
			       const std::map<std::string /*name*/, std::string /*alias*/>&
			                                          memberTableAliases,
			       std::atomic<bool>*                 theFoundIdentical,
			       ots::TableGroupKey*                theIdenticalKey,
			       std::mutex*                        theThreadMutex,
			       std::shared_ptr<std::atomic<bool>> theThreadDone) {
				    ConfigurationManagerRW::compareTableGroupThread(cfgMgr,
				                                                    theGroupName,
				                                                    groupKeyToCompare,
				                                                    groupMemberMap,
				                                                    memberTableAliases,
		...
		do
		{
			...
			if(foundThreadIndex != -1)
			{
				__GEN_COUTT__ << "Waiting for thread to finish... " << foundThreadIndex
				              << __E__;
				usleep(10000);
			}
		} while(foundThreadIndex != -1);  //end thread done search loop
    ...
    std::map<std::string /*table*/, std::string /*alias*/>* groupAliases)
{
	...
	for(auto& memberPair : groupMembers)
	{
		...
		{
			__GEN_COUT_ERR__ << "Group member \"" << memberPair.first
	...
	if(groupAliases)
	{
		for(auto& aliasPair : *groupAliases)
		{
			...
			{
				__GEN_COUT_ERR__ << "Group member \"" << aliasPair.first
...
TableVersion ConfigurationManagerRW::saveNewBackbone(TableVersion temporaryVersion)
{
	...
	for(auto& name : backboneMemberNames)
	{
		...
		__GEN_COUT__ << "New version for backbone member (" << name
		             << "): " << retNewVersion << __E__;
    ...
    bool*              foundEquivalent /*= nullptr*/)
{
	...
	if(!ignoreDuplicates)
	{
...

		{
			...
			__GEN_COUT__ << "Filling up '" << tableName << "' cache from "
			...
			{
				__GEN_COUTT__ << "'" << tableName << "' versions in reverse order "
		...
		{
			// found an equivalent!
			__GEN_COUT__ << "Equivalent '" << tableName << "' table found in version v"
			...
			else
			{
				...
				__GEN_COUT__ << "\t\t Equivalent '" << tableName
		...
		__GEN_COUT__ << "Check for duplicate '" << tableName << "' tables complete."
		             << __E__;
	}

	if(makeTemporary)
		__GEN_COUT__ << "\t\t**************************** Save as temporary '"
		             << tableName << "' table version" << __E__;
	else
		__GEN_COUT__ << "\t\t**************************** Save as new '" << tableName
		             << "' table version" << __E__;
    ...
    , mfSubject_(cfgMgr->getUsername())
{
	...
	__GEN_COUT__ << "Extracting Group-Edit Struct for type "
	...
	for(auto& memberName : memberNames)
		try
		{
			groupMembers_.emplace(
			    std::make_pair(memberName, activeTables.at(memberName)));
			groupTables_.emplace(std::make_pair(
			    memberName,
...
{
	__GEN_COUT__ << "GroupEditStruct from editing '" << originalGroupName_ << "("
                                                     ...
                                                     bool markModified /*= false*/)
{
	...
	if(it == groupTables_.end())
	{
		if(groupType_ == ConfigurationManager::GroupType::CONFIGURATION_TYPE &&
		   markModified)
		{
			__GEN_COUT__ << "Table '" << tableName
			             << "' not found in configuration table members from editing '"
			             << originalGroupName_ << "(" << originalGroupKey_ << ")..."
			             << " Attempting to add it!" << __E__;

			// emplace returns pair<object,bool wasAdded>
			auto newIt = groupTables_.emplace(std::make_pair(
			    tableName,
			    TableEditStruct(tableName, cfgMgr_)));  // Table ready for editing!
			if(newIt.second)
			{
				newIt.first->second.modified_ =
				    markModified;  // could indicate 'dirty' immediately in user code, which will cause a save of table
				groupMembers_.emplace(
				    std::make_pair(tableName, newIt.first->second.temporaryVersion_));
		...
		__SS__ << "Table '" << tableName << "' not found in table members from editing '"
		       << originalGroupName_ << "(" << originalGroupKey_ << ")!'" << __E__;
		__SS_THROW__;
	}
	it->second.modified_ =
	    markModified;  // could indicate 'dirty' immediately in user code, which will cause a save of table
...
{
	__GEN_COUT__ << "Dropping unsaved changes from editing '" << originalGroupName_ << "("
                                  ...
                                  std::string*   accumulatedWarnings /*= nullptr*/)
{
	...
	bool anyTableNew = false;
	for(auto& groupTable : groupTables_)
	{
		if(!groupTable.second.modified_)
			continue;  // skip if not modified

		__GEN_COUT__ << "Original version is " << groupTable.second.tableName_ << "-v"
		             << groupTable.second.originalVersion_ << __E__;

		groupMembers_.at(groupTable.first) = cfgMgr->saveModifiedVersion(
		...
		__GEN_COUT__ << "Temporary target version is " << groupTable.second.tableName_
		             << "-v" << groupMembers_.at(groupTable.first) << "-v"
	...
	if(!anyTableNew)  //then could be duplicate group
	{
		...
		__GEN_COUT__ << "New table found, so no need to check duplicate groups." << __E__;

	if(!newGroupKey.isInvalid())
	{
		__GEN_COUT__ << "Found equivalent group key (" << newGroupKey << ") for "
	...
	else
	{
		...
		__GEN_COUT__ << "Saved new Context group key (" << newGroupKey << ") for "
		             << groupNameToSave << "." << __E__;
	}
	...
	try
	{
		//test lookup of which groups a table is in
		{
			...
			TableVersion documentVersionToLoad(
			    (int)134);  //1 is easy, 134 is hard on daq13 mongodb
