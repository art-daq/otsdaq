...
class ConfigurationManagerRW : public ConfigurationManager
{
  public:
	...
	template<class T>
	T* getTablePtr(const std::string& tableName)
	{
	...
	TableBase* getMetadataTable(
	    TableVersion fillVersion =
	        TableVersion());  ///< created for use in otsdaq_flatten_system_aliases and otsdaq_export_system_aliases, e.g.
	...
	TableVersion saveModifiedVersion(const std::string& tableName,
	...
	void activateTableGroup(const std::string& tableGroupName,
	...
	const GroupInfo&                        getGroupInfo(const std::string& groupName,
	                                                     bool               attemptToReloadKeys = false);
  ...
  public:
	...
	static void compareTableGroupThread(
...
struct TableEditStruct
{
	...
	TableEditStruct(const std::string&      tableName,
	                ConfigurationManagerRW* cfgMgr,
	    ...
	    , mfSubject_(cfgMgr->getUsername())
	{
		   ...
		   !(originalVersion_ = table_->getView().getVersion()).isTemporaryVersion())
		{
			...
			__COUT__ << "Created '" << tableName_ << "' temporary version "
			         << temporaryVersion_ << std::endl;
...
struct GroupEditStruct
{
  ...
  public:
	/////
	GroupEditStruct() : groupType_(ConfigurationManager::GroupType::CONFIGURATION_TYPE)
	{
		__SS__ << "impossible!" << __E__;
		__SS_THROW__;
	}
	GroupEditStruct(const ConfigurationManager::GroupType& groupType,
	                ConfigurationManagerRW*                cfgMgr);

	~GroupEditStruct();

	void dropChanges(void);
	void saveChanges(const std::string& groupNameToSave,
	...
	TableEditStruct& getTableEditStruct(const std::string& tableName,
	                                    bool               markModified = false);
