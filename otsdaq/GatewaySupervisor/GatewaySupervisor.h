                          ...
                          public ConfigurationSupervisorBase
{
  ...
  public:
	...
	static bool        handleAddDesktopIconRequest(
	           const std::string&                          author,
	           cgicc::Cgicc&                               cgiIn,
	           HttpXmlDocument&                            xmlOut,
	           std::vector<DesktopIconTable::DesktopIcon>* newIcons = nullptr);
	static void handleGetApplicationIdRequest(
	    AllSupervisorInfo*                                        applicationInfo,
	    cgicc::Cgicc&                                             cgiIn,
	    HttpXmlDocument&                                          xmlOut,
  ...
  private:
	...
	std::string getLastLogEntry(const std::string& logType,
	                            const std::string& fsmName = "");
	void        setLastLogEntry(const std::string& logType,
	                            const std::string& logEntry,
	                            const std::string& fsmName = "");

	static xoap::MessageReference lastTableGroupRequestHandler(
	    const SOAPParameters& parameters);
	static void launchStartOTSCommand(const std::string&    command,
	                                  ConfigurationManager* cfgMgr);
	static void launchStartOneServerCommand(const std::string&    command,
	                                        ConfigurationManager* cfgMgr,
	...
	bool
	    supervisorGuiHasBeenLoaded_;  ///< use to indicate first access by user of ots since execution
	...
	std::string
	    activeStateMachineName_;  ///< when multiple state machines, this is the name of the state machine which executed the configure transition
	std::string activeStateMachineWindowName_;
	std::string activeStateMachineDumpFormatOnRun_,
	    activeStateMachineDumpFormatOnConfigure_;  ///<cached at Configure transition
	std::string activeStateMachineSystemDumpOnRun_,
	    activeStateMachineSystemDumpOnConfigure_;  ///<cached at Configure transition
	bool activeStateMachineSystemDumpOnRunEnable_,
	    activeStateMachineSystemDumpOnConfigureEnable_;  ///<cached at Configure transition
	std::string activeStateMachineSystemDumpOnRunFilename_,
	    activeStateMachineSystemDumpOnConfigureFilename_;  ///<cached at Configure transition
	bool activeStateMachineRequireUserLogOnRun_,
	    activeStateMachineRequireUserLogOnConfigure_;  ///<cached at Configure transition
	...
	std::string lastConsoleErr_, lastConsoleWarn_, lastConsoleInfo_, lastConsoleErrTime_,
	    lastConsoleWarnTime_, lastConsoleInfoTime_;
	std::string firstConsoleErr_, firstConsoleWarn_, firstConsoleInfo_,
	    firstConsoleErrTime_, firstConsoleWarnTime_, firstConsoleInfoTime_;
	size_t systemConsoleErrCount_ = 0, systemConsoleWarnCount_ = 0,
	       systemConsoleInfoCount_ = 0;
	...
	std::string
	    stateMachineTransitionUsername_;  ///< used to track the user who made the last state machine transition (for logging purposes)
	...
	unsigned int
	    broadcastIterationBreakpoint_;  ///< pause transition when iteration index
	                                    ///< matches breakpoint index
	std::mutex  broadcastCommandStatusUpdateMutex_;
	std::string broadcastCommandStatus_;
	static std::vector<std::shared_ptr<GatewaySupervisor::BroadcastThreadStruct>>
	    broadcastThreadStructs_;  ///<moving to static, instead of a local instance inside broadcastMessage() seems to avoid crashing when multiple error stack up and threads get stuck waiting for app replies
  ...
  public:  //used by remote subsystem control and status
	struct RemoteGatewayInfo
	{
		...
		enum class
		    ConfigDumpTypes  ///<FSM Modes: 'Follow FSM,' 'Do not Halt' (artdaq),  or 'Only Configure' (DCS/DQM)
		{
			Text,
			JSON_all,
			Unknown
	  ...
	  public:
		...
		void copyError(const RemoteGatewayInfo& r)
		{
			error          = r.error;
			errorTimestamp = r.errorTimestamp;
		}
		const std::string& getError() const { return error; }
		const std::string  getErrorTimestamp() const
		{
			return StringMacros::getTimestampString(errorTimestamp);
		}
		...
		std::string setupType, instancePath, instanceHost,
		    instanceUser;  ///<used for remote ots instance ssh launch
		...
		std::string           iconString, parentIconFolderPath, landingPage,
		    permissionThresholdString;  ///<used for desktop icons

		std::string usernameWithLock;

		enum class
		    FSM_ModeTypes  ///<FSM Modes: 'Follow FSM,' 'Do not Halt' (artdaq),  or 'Only Configure' (DCS/DQM)
		{
			Follow_FSM,
		...
		FSM_ModeTypes fsm_mode =
		    FSM_ModeTypes::Follow_FSM;  ///< used for remote gateway subapp control
		...
		std::string getFsmMode() const
		{
			switch(fsm_mode)
			{
			case FSM_ModeTypes::Follow_FSM:
				return "Follow FSM";
			case FSM_ModeTypes::DoNotHalt:
				return "Do Not Halt";
			case FSM_ModeTypes::OnlyConfigure:
				return "Only Configure";
			default:
				return "Impossible";
			}
		}  //end getFsmMode()
		...
		std::string getConfigDumpType() const
		{
			...
			{
			case ConfigDumpTypes::Text:
				return "Text";
			case ConfigDumpTypes::JSON_all:
				return "JSON all";
			default:
				return "Unknown";
			}
		...
		std::map<std::string, SupervisorInfo::SubappInfo>
		    subapps;  ///< remote gateways can have subapps
	...
	std::map<std::string /* appName */, bool /* lastStatusGood */> appLastStatusGood_;
	std::mutex                                                     dualStatusThreadMutex_;

	std::map<unsigned int /* lid */, SupervisorInfo>
	    localAllSupervisorInfo_;  ///< only use in main thread, stable copy of app status

	std::mutex latestGatewayIconsMutex_;
	std::vector<DesktopIconTable::DesktopIcon>
	    latestGatewayIcons_;  ///< used to track the latest desktop icons (which are defined by the active context but allowed to change dynamically)
	...
	std::string
	    latestGatewayRemoteIconsString_;  ///< cached string of remote gateway icons for quick access
	std::pair<std::string /* latestIconContext group */, TableGroupKey>
	    latestGatewayRemoteIconsContextGroup_;  ///< used to track the table group key for the latest remote desktop icons

	static void CheckRemoteGatewayStatus(
	...
	static void SendRemoteGatewayCommand(
	    GatewaySupervisor::RemoteGatewayInfo&     remoteGatewayApp,
	    const std::unique_ptr<TransceiverSocket>& remoteGatewaySocket);
	static void GetRemoteGatewayIcons(
	    GatewaySupervisor::RemoteGatewayInfo&     remoteGatewayApp,
	    const std::unique_ptr<TransceiverSocket>& remoteGatewaySocket);
	void loadRemoteGatewaySettings(
	...
	static std::string translateURLForRequestOrigin(
	    const std::string&                                        url,
	    const std::string&                                        requestOrigin,
	    std::map<std::string /* requestOrigin */,
	             std::map<std::string /* requestUrlHostPort */,
	                      std::string /* translatedHostPort */>>& portTranslationMap);
	static std::string translateRemoteIconStringForRequestOrigin(
