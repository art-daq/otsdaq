					   ...
					   icon.windowContentURL_[3] == ':')
					{
						...
						if(!found)
						{
							__SUP_SS__ << "Illegal missing remote icon definition for "
							              "icon record UID '"
							           << icon.recordUID_ << ".' If the corresponding subsystem should exist, perhaps Gateway Application Status Monitoring is disabled. It must be enabled for subsystem management. Please notify admins if the problem persists."
							           << __E__;
							__SUP_SS_THROW__;
						}
