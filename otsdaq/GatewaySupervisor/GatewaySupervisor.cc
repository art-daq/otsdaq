...
try
{
	...
	// Safety check: blank fsmName means the client request is corrupt (e.g., an
	// HTML/JavaScript bug that fails to include the fsmName parameter). Reject it
	// immediately so that downstream code (e.g., getNextRunNumber) never silently
	// uses the wrong run-number file.
	if(fsmName == "")
	{
		__SS__ << "Error - The State Machine name (fsmName) in the '" << command
		       ...
		       << "'. Please verify the client is sending the fsmName parameter."
		       << __E__;
		__SS_THROW__;
	}
