...
try
{
	       ...
	       .size())
	{
		...
		xoap::MessageReference replyMessage =
		    SOAPMessenger::sendWithSOAPReply(CorePropertySupervisorBase::allSupervisorInfo_
		                            .getAllMacroMakerTypeSupervisorInfo()
		                            .begin()
		                            ->second.getDescriptor(),
		                        message);
		std::string reply = SOAPUtilities::receive(replyMessage); //get primary message response
